...
<xsl:template match="data">
  <rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">
    <channel>
      ...
      <xsl:for-each select="rss-blogs/entry">
        <item>
          <title><xsl:value-of select="title"/></title>
          <link><xsl:value-of select="$root"/>/blogs/<xsl:value-of select="title/@handle"/>/</link>
          <pubDate>
          ...
          <guid><xsl:value-of select="$root"/>/blogs/<xsl:value-of select="title/@handle"/>/</guid>
