...
<?php e($director->preDialogue('embed-code', false, 840)); ?>
						...
						<h4><?php __('Slideshow options') ?></h4>
							...
							<fieldset>
								<label class="inline"><?php __('Style') ?>:</label>&nbsp;&nbsp;<select id="embed-style" onchange="update_embed_code(true);">
									...
									?>
									<option value="<?php e($t); ?>"<?php e(ife($display == 'Default', ' selected="selected"')); ?>><?php e($display); ?></option>
									<?php endforeach; ?>
								</select>
							</fieldset>
	...
	<span class="messenger-bttns" style="margin-top:0;"><button onclick="Messaging.kill('embed-code');" class="primary_lg nudgeR" title="<?php __('Done') ?>"><?php __('Done') ?></button><div id="<?php e($eid); ?>" style="float:right;"><textarea id="<?php e($eid); ?>_tocopy" style="display:none;"></textarea><button id="<?php e($eid) ?>_button" type="button" title="<?php __('Copy'); ?>" class="primary_lg"><?php __('Copy embed code'); ?></button><div id="<?php e($eid) ?>_target" style="position:absolute;top:0;left:0;z-index:1500;"></div></div></span> 
